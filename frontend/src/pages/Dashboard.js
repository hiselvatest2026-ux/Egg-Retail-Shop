...
const Dashboard = () => {
  ...
  const salesTrendBar = useMemo(() => {
    const rawLabels = data?.sales_trend?.map(d => d.day) ?? [];
    const labels = rawLabels.map(formatDay);
    ...
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <Card title="Revenue by Customer Type (Daily)">
          <div style={{ background:'#ffffff', borderRadius:12, padding:12 }}>
            <Bar data={revenueByCategoryChart} options={{ responsive: true, plugins: { legend: baseLegend }, scales: { x: { stacked:true, ticks: { autoSkip: false } }, y: { stacked:true, beginAtZero:true } } }} />
          </div>
        </Card>
        <Card title="Qty by Customer Type (Daily)">
          <div style={{ background:'#ffffff', borderRadius:12, padding:12 }}>
            <Bar data={qtyByCategoryChart} options={{ responsive: true, plugins: { legend: baseLegend }, scales: { x: { stacked:true, ticks: { autoSkip: false } }, y: { stacked:true, beginAtZero:true } } }} />
          </div>
        </Card>
      </div>
